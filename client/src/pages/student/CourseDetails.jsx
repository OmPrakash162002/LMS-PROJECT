 ...
 try {
  ...
  return courseData ? (
    ...
    <div className="flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left">
      <div className="absolute top-0 left-0 w-full -z-1 bg-gradient-to-b from-red-300 to-white h-screen"></div>

      {/* left column */}
      <div className="max-w-xl z-10 text-gray-500">
        <h1 className="font-bold text-3xl text-black">{courseData.courseTitle}</h1>
        <p className="pt-3"
        ...
        {/* review and ratings */}
        <div className="flex flex-row items-center gap-2 text-xs pt-3 pb-3">
        ...
        <p className="text-xs">Course by <span className="text-blue-600 underline">{courseData.educator.name}</span></p>
        {/* course stucture div */}
        <div className="mt-15">
        ...
        {/* course descreption div */}
        <div className="py-20 text-sm md:text-default">
          <h3 className="text-xl font-semibold text-gray-800 ">Course Description</h3>
          <p className="pt-3 list-disc pl-5"
          dangerouslySetInnerHTML={{
      ...
      <div className="bg-white shadow-2xl pb-3 ">
        ...
        <div className="pt-3 flex flex-row gap-2 items-center px-3 ">
          <img className="w-3.5" src={assets.time_left_clock_icon} alt="time left clock icon" />
          <p className="text-red-500"><span className="font-medium">5 days </span>left at this price!</p>
        </div>
        <div className="flex flex-row items-center gap-3 px-3">
          <p className="font-semibold text-2xl">{currency}{(courseData.coursePrice - courseData.discount * courseData.coursePrice / 100).toFixed(2)}</p>
          <p className="text-gray-600 line-through">{currency}{courseData.coursePrice}</p>
          <p className="text-gray-600">{courseData.discount}% off</p>
        </div>

        <div className="flex flex-row gap-4 pt-3 items-center px-3">
          <div className="flex flex-row gap-1 items-center">
            <img className="h-3" src={assets.star} alt="star icon" />
